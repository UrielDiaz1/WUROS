...
void pic_irq_enable(int irq) {
    ...
    // Remaps the PIC interrupts so we can register them.
    if(irq >= 0x20) {
        irq -= 0x20;
    }
...
void pic_irq_dismiss(int irq) {
    ...
    // EOI needs to be send to both pics because they're daisy-chained.
    // IRQ 8 through 15 are mapped to PIC2.
    if (irq >= 0x8) {
        outportb(PIC2_CMD, PIC_EOI);
        outportb(PIC1_CMD, PIC_EOI);
    }
    // Send EOI to the primary PIC, if needed.
    // IRQ 0 through 7 are mapped to PIC1.
    else {
        outportb(PIC1_CMD, PIC_EOI);
    }
}
